...
def run_csda_on_demos(tournaments_df):
    ...
    for tournament, group in tournaments_df.groupby('tournament'):
        ...
        row = group.iloc[0]
        csda_path = os.path.join(row['root'], "csda.exe")
        demos_path = row['demo_path']
        output_path = os.path.join(row['root'], "extracted_data", tournament)
        ...
        if os.path.exists(os.path.join(row['root'], 'downloads')):
            shutil.rmtree(demos_path)        
