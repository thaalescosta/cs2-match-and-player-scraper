...
def merge_csv_files(tournaments_df):
    ...
    columns_to_drop = {
        ...
        "_kills.csv": ["frame", "tick", "killer_side", "assister_name", "assister_steamid", "assister_side", "assister_team_name", "weapon_type", "penetrated_objects", "is_flash_assist", "killer_controlling_bot", "victim_controlling_bot", "assister_controlling_bot", "killer_x", "killer_y", "killer_z", "is_killer_airborne", "is_killer_blinded", "is_victim_airborne", "is_victim_blinded", "is_victim_inspecting_weapon", "assister_x", "assister_y", "assister_z", "is_trade_death", "is_through_smoke", "is_no_scope", "distance", "source_file"],
        "_match.csv": ["game", "demo_path", "type", "share_code", "server_name", "client_name", "tick_count", "tickrate", "framerate", "game_type", "game_mode", "game_mode_str", "is_ranked", "duration", "network_protocol", "build_number", "shot_count", "winner_name", "winner_side", "overtime_count", "max_rounds", "has_vac_live_ban"],
    ...
    for tournament, group in tournaments_df.groupby('tournament'):
        ...
        for suffix, column in columns_to_drop.items():
            ...
            if suffix == "_teams.csv":
                ...
                merged_df["unique_name"] = merged_df["demo_name"] + "_" + merged_df["tournament"].str.lower().str.replace(" ", "-")
    ...
    tournaments_df.drop(columns={'root', 'url_request', 'direct_url_demo', 'file_name'}, inplace=True)
    tournaments_df.rename(columns={'url': 'url_match'})      
