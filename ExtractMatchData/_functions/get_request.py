...
def get_request(tournaments_df):
    # Create a session once, to reuse for each match
    ...
    # Loop through the DataFrame and make the request
    for i, match in enumerate(tournaments_df['match_id']):
        try:
            ...
            tournaments_df.at[i, 'url_request'] = url
            tournaments_df.at[i, 'url_demo'] = response.headers.get("Location", "No download link found")
            
        
        except Exception as e:
            print(f"Error fetching data for match {match}: {e}")
